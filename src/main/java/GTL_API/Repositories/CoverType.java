...
public class CoverType implements ICoverTypeCustom {
    ...
    public CoverTypeReturn updateCoverType(CoverTypeEntity coverTypeEntity) {
        try{
            ...
            checkIfExistsByCoverType(found, "U");
    ...
    //"U" is updating
    //"A" is adding
    private void checkIfExistsByCoverType(CoverTypeEntity coverTypeEntity, String operation){
        if(operation == "U"){
            if(iCoverType.countAllByCoverTypeIs(coverTypeEntity.getCoverType()) > 1){
            ...
            if(iCoverType.countAllByCoverTypeIs(coverTypeEntity.getCoverType()) > 0){
                throw new DuplicateException("There is already record of cover type of name: " + coverTypeEntity.getCoverType() + ".");
            }
        }
    }
}
